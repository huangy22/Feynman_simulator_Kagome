            ...
            for i in range(3):
                for j in range(3):
                    for e in range(Lx*Ly):
                        coord = self.__Map.IndexToCoordi(e)
                        dist,vec = self.Lat.GetRealDistance(coord, i, j)
                        #if i == 2 and j == 1:
                            #print coord, vec, dist
                        if not(i==j and e==0):
                            #self.BareW.Data[:,i,:,j,e]+= (J1/dist**3.0)*SS;
                            self.BareW.Data[:,i,:,j,e]+= (J1/dist**3.0)*SzSz;
